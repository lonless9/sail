...
def local_cluster_server():
    """Start a SparkConnectServer in local-cluster mode."""
...
@pytest.fixture(scope="module")
def local_cluster_spark(local_cluster_server):
    spark = SparkSession.builder.remote(local_cluster_server).getOrCreate()
...
def large_dataset(local_cluster_spark):
    ...
    df = local_cluster_spark.createDataFrame(total)
    df.createOrReplaceTempView("large_dataset")
    yield df
    local_cluster_spark.catalog.dropTempView("large_dataset")


@pytest.mark.skipif(is_jvm_spark(), reason="Sail local-cluster mode only")
class TestLocalClusterExecution:
    def test_basic_query_execution(self, local_cluster_spark):
        """Test basic query execution in local-cluster mode."""
        result = local_cluster_spark.sql("SELECT 1 + 1 AS result").collect()
        assert result[0]["result"] == 2  # noqa: PLR2004

    def test_dataframe_operations(self, local_cluster_spark):
        """Test DataFrame operations in local-cluster mode."""
        df = local_cluster_spark.createDataFrame([Row(a=1, b="hello"), Row(a=2, b="world"), Row(a=3, b="test")])
    ...
    def test_join_operations(self, local_cluster_spark):
        ...
        customers = local_cluster_spark.createDataFrame(
            [
        ...
        orders = local_cluster_spark.createDataFrame(
            [
    ...
    def test_complex_sql_query(self, local_cluster_spark):
        ...
        df = local_cluster_spark.createDataFrame(sales_data)
        df.createOrReplaceTempView("sales")

        result = local_cluster_spark.sql("""
        ...
        local_cluster_spark.catalog.dropTempView("sales")

    def test_multiple_concurrent_operations(self, local_cluster_spark):
        """Test that multiple operations can run concurrently in local-cluster mode."""
        df1 = local_cluster_spark.range(100).select(F.col("id").alias("id1"))
        df2 = local_cluster_spark.range(100).select(F.col("id").alias("id2"))
